...
import { RawAesKeyringWebCrypto, RawAesWrappingSuiteIdentifier } from '../src/index'
import {
...
describe('importCryptoKey', () => {
  ...
  it('Precondition: masterKey must correspond to the algorithm suite specification.', async () => {
    ...
    expect(RawAesKeyringWebCrypto.importCryptoKey(notValidLength, suiteId)).to.rejectedWith(Error)
  })
})
...
describe('RawAesKeyringWebCrypto::constructor', () => {
  ...
  it('constructor decorates', async () => {
    ...
    const test = new RawAesKeyringWebCrypto({ keyName, keyNamespace, masterKey, wrappingSuite })
  ...
  it('Precondition: AesKeyringWebCrypto needs identifying information for encrypt and decrypt.', async () => {
    const masterKey = await RawAesKeyringWebCrypto.importCryptoKey(udk, wrappingSuite)
    // @ts-ignore doing something typescript protects us from doing...s
    expect(() => new RawAesKeyringWebCrypto({ keyNamespace, masterKey, wrappingSuite })).to.throw()
    // @ts-ignore doing something typescript protects us from doing...s
...
describe('RawAesKeyringWebCrypto::_filter', () => {
  ...
  before(async () => {
    const masterKey = await RawAesKeyringWebCrypto.importCryptoKey(udk, wrappingSuite)
    keyring = new RawAesKeyringWebCrypto({ keyName, keyNamespace, masterKey, wrappingSuite })
  })
...
describe('RawAesKeyringWebCrypto encrypt/decrypt', () => {
  ...
  before(async () => {
    const masterKey = await RawAesKeyringWebCrypto.importCryptoKey(masterUdk, wrappingSuite)
    keyring = new RawAesKeyringWebCrypto({ keyName, keyNamespace, masterKey, wrappingSuite })
  })
