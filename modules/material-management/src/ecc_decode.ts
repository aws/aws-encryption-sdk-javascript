...
const secp384r1 = eccDecodeCompressedPoint(
  new BN('FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFFFF0000000000000000FFFFFFFF', 16),
  new BN('FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFFFF0000000000000000FFFFFFFC', 16),
  new BN('B3312FA7E23EE7E4988E056BE3F82D19181D9C6EFE8141120314088F5013875AC656398D8A2ED19D2A85C8EDD3EC2AEF', 16),
  // new BN('FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFC7634D81F4372DDF581A0DB248B0A77AECEC196ACCC52973', 16)
)

type decodeNamedCurves = {[K in NodeECDHCurve|WebCryptoECDHCurve]: (compressedPoint: Uint8Array) => Uint8Array}
...
function eccDecodeCompressedPoint(p: BN, a: BN, b: BN/*, order: BN*/) {
  ...
  return function decode(compressedPoint: Uint8Array) {
    ...
    if (compressedPoint.byteLength !== compressedLength) throw new Error('')
