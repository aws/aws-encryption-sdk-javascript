...
export abstract class AlgorithmSuite implements IAlgorithmSuite {
  ...
  constructor(suite: INodeAlgorithmSuite|IWebCryptoAlgorithmSuite) {
    if (this.constructor === AlgorithmSuite) {
      throw new Error(`new AlgorithmSuite is not allowed`)
    }
    if (!suite) throw new Error('')
    if (typeof suite.id !== 'number' || !AlgorithmSuiteIdentifier[suite.id]) throw new Error('')
