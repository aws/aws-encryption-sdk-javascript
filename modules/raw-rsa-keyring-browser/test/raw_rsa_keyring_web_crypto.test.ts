...
describe('import CryptoKey helpers', () => {
  it('imports public CryptoKey', async () => {
    const cryptoKey = await RawRsaKeyringWebCrypto.importPublicKey(publicRsaJwkKey)
    expect(cryptoKey).to.be.instanceOf(CryptoKey)
  })

  it('imports private CryptoKey', async () => {
    const cryptoKey = await RawRsaKeyringWebCrypto.importPrivateKey(privateRsaJwkKey)
    expect(cryptoKey).to.be.instanceOf(CryptoKey)
  })
})
...
describe('RawRsaKeyringWebCrypto::constructor', () => {
  ...
  before(async () => {
    publicCryptoKey = await await RawRsaKeyringWebCrypto.importPublicKey(publicRsaJwkKey)
    privateCryptoKey = await await RawRsaKeyringWebCrypto.importPrivateKey(privateRsaJwkKey)
  })
  ...
  it('Precondition: RsaKeyringWebCrypto needs either a public or a private key to operate.', () => {
    expect(() => new RawRsaKeyringWebCrypto({
      keyName,
      keyNamespace
    })).to.throw()
  })
...
describe('RawRsaKeyringWebCrypto encrypt/decrypt', () => {
  ...
  before(async () => {
    const publicKey = await await RawRsaKeyringWebCrypto.importPublicKey(publicRsaJwkKey)
    const privateKey = await await RawRsaKeyringWebCrypto.importPrivateKey(privateRsaJwkKey)
    keyring = new RawRsaKeyringWebCrypto({ publicKey, privateKey, keyName, keyNamespace })
  })
